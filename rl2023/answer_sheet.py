...
def question2_4() -> str:
    ...
    answer = "Gamma affects First visit MC more (mean of -78 vs 7.5) than Q-Learning (mean of -1.38 vs 8.02). In MC we calculate the return for the whole lenght of episode, thus the return at the terminal state will be multiplied with gamma many times, before it reaches the first visit. In Q learning we only look at the next time step, so the gamma is only multiplied onces thus have less impact "

    # answer = "first visit monte carlo goes through an entire episode to update the q_table, where gamma tells you how much focus is there on long term rewards. Whereas in Q-learning, we update after 1 step, thus the value of gamma has less effect as it is only multiplied by itself once In monte carlo it multiplied by itself many times, thus small changes are increased"  # TYPE YOUR ANSWER HERE (100 words max)
    # answer += '. For example in Monte Carlo if gamma is 0.8, then reward of 50th step will be gamma^50 * reward[50], and gamma is [0,1], if close to one then value is less affected, with lower then impact is high'
    return answer
