    ...
    **YOU NEED TO IMPLEMENT FUNCTIONS IN THIS CLASS**
    ...
    def schedule_hyperparameters(self, timestep: int, max_timestep: int):
        ...
        ** You may modify the signature of these functions if you wish to pass additional arguments **
        ...
        def epsilon_linear_decay(epsilon_start,epsilon_end,exp_frac):
            ### PUT YOUR CODE HERE ###

            decay_timesteps = int((1 - exp_frac) * max_timestep)
            current_fraction = timestep/max_timestep

            if current_fraction<= exp_frac:
                decay = (epsilon_start-epsilon_end)/decay_timesteps
                epsilon = epsilon_start - decay
            else:
                epsilon = epsilon_end

            return epsilon

        def epsilon_exponential_decay():
            ### PUT YOUR CODE HERE ###
            
            return max(self.epsilon_min, self.epsilon_start*(self.epsilon_exponential_decay_factor ** (timestep / max_timestep)))
    ...
    def act(self, obs: np.ndarray, explore: bool):
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        # get the q_tables network values
        action_values = self.critics_net(torch.from_numpy(obs).float()).detach().numpy()

        if explore == True and np.random.random() < self.epsilon:
            # we will return exploration i.e. random action
    ...
    def update(self, batch: Transition) -> Dict[str, float]:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        # print()
        # print(done.shape, actions.shape, rewards.shape, states.shape, next_states.shape)
        # print(self.batch_size)
        # print()
        # so we have the sampled batch, i.e. here it is 64 size.

        # checking if the state is terminal:
        

        # getting yj from Q' (target)
        qsmax = self.critics_target(next_states).max(1)[0].detach().unsqueeze(1) # the highest qs for an action
        yj = rewards + (1 - done) * self.gamma * qsmax # 1 -done will tell us if the current state is ternimal


        # getting value from Q (action value function)
        value = self.critics_net(states).gather(dim=1, index=actions.long())  
        ...
        if self.update_counter % self.target_update_freq == 0:
            self.critics_target.hard_update(self.critics_net) # Q' = Q
    ...
    ** YOU NEED TO IMPLEMENT THE FUNCTIONS IN THIS CLASS **
    ...
    def act(self, obs: np.ndarray, explore: bool):
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        # get the actions from policy given the observeration we have (obs)
        obs_tensor = torch.from_numpy(obs).float() # as policy forwards takes in a tensor
        action_prob = self.policy.forward(obs_tensor).detach().numpy() #getting the action probs of the policy of that obs, 

        # these probs are already normalized, that sum is one
        actions = np.arange(self.action_space.n) # list of actions in numpy form

        if explore:
            # this is mostly during training, so we need t
            return np.random.choice(actions, p=action_prob) # selecting action with probs by policy

        else:
            # no explore, just return a greedy action
            return np.argmax(action_prob) # best action 
    ...
    def update(
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        # telling the model that it is in train mode 
        self.policy.train()
        ...
        for t in reversed(range(T)):
            ...
            p_loss -= G * torch.log(self.policy.forward(torch.from_numpy(observations[t])).float()[actions[t]])

        p_loss /= T

        self.policy_optim.zero_grad() # setting the gradients to zero 
