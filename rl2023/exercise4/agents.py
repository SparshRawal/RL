...
class DDPG(Agent):
    ...
    def act(self, obs: np.ndarray, explore: bool):
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q4**
        ...
        """
        ### PUT YOUR CODE HERE ###
        action = self.actor_target.forward(torch.from_numpy(obs).float()).detach()
        if explore:
            noise = self.noise.sample()
            action += noise

        action = np.clip(action, self.action_space.low, self.action_space.high) # noise can make action invalid
        return action
